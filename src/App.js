...
const App = () => {
  ...
  const smartConsumption = useMemo(() => {
    return feeds.map(feed => {
      ...
      if (intervals.length === 0) return { ...feed, purchases, intervals: [], avgDailyBags: null, avgDailyKg: null, lastPurchaseDate: null, nextPurchaseEstimate: null, daysUntilNext: null, confidence: 'low', needsAlert: false, weeklyCost: null, monthlyCost: null, yearlyCost: null };

      // متوسط مرجّح — الأحدث أكثر وزناً
      const totalWeight = intervals.reduce((s, _, i) => s + (i + 1), 0);
      ...
      const monthlyCost = pricePerBag > 0 ? weightedAvgBags * 30 * pricePerBag : null;
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', zIndex: 1000, padding: '15px', overflowY: 'auto' }} onClick={() => setShowFeedSystem(false)}>
          <div style={{ background: 'white', borderRadius: '14px', maxWidth: '780px', width: '100%', marginTop: '15px', overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.4)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ maxHeight: '580px', overflowY: 'auto', padding: '15px 20px' }}>
              ...
              {/* ===== تبويب الاستهلاك الذكي ===== */}
              {feedTab === 'smart' && (
                <div>
                  {/* شرح الطريقة */}
                  <div style={{ background: '#f0f4ff', borderRadius: '10px', padding: '12px 15px', marginBottom: '15px', border: '1px solid #c5cae9', fontSize: '12px', color: '#2471a3' }}>
                    <div style={{ fontWeight: 'bold', marginBottom: '5px', fontSize: '13px' }}>🧠 كيف يعمل الحساب الذكي؟</div>
                    <div style={{ lineHeight: '1.8', color: '#555' }}>
                      يحسب الاستهلاك الفعلي من سجل مشترياتك — <strong>كمية الشراء السابقة ÷ عدد الأيام حتى الشراء التالي</strong><br/>
                      المتوسط مرجّح للأحدث أكثر دقة · يحتاج عمليتي شراء على الأقل لكل نوع علف
                    </div>
                  </div>

                  {/* تنبيهات */}
                  {smartConsumption.some(f => f.needsAlert) && (
                    <div style={{ background: '#fff3f3', border: '2px solid #e74c3c', borderRadius: '10px', padding: '12px 15px', marginBottom: '15px' }}>
                      <div style={{ fontWeight: 'bold', color: '#e74c3c', fontSize: '13px', marginBottom: '6px' }}>⚠️ أعلاف تحتاج شراء قريباً</div>
                  ...
                  {/* بطاقة لكل علف */}
                  {smartConsumption.map(feed => {
                    const hasEnoughData = feed.intervals && feed.intervals.length >= 1;
                    const confidenceColor = feed.confidence === 'high' ? '#27ae60' : feed.confidence === 'medium' ? '#e67e22' : '#bbb';
                    const confidenceLabel = feed.confidence === 'high' ? '✅ دقة عالية' : feed.confidence === 'medium' ? '⚠️ دقة متوسطة' : '❓ بيانات غير كافية';
                    const pct = hasEnoughData && feed.estimatedDaysTotal > 0
                      ? Math.max(0, Math.round(((feed.estimatedDaysTotal - feed.daysFromLastPurchase) / feed.estimatedDaysTotal) * 100))
                      : null;
                    const barColor = pct === null ? '#bbb' : pct > 40 ? '#27ae60' : pct > 20 ? '#e67e22' : '#e74c3c';

                    return (
                      <div key={feed.id} style={{ background: feed.needsAlert ? '#fff8f0' : 'white', border: `1.5px solid ${feed.needsAlert ? '#e67e22' : '#eee'}`, borderRadius: '12px', padding: '14px', marginBottom: '12px' }}>
                        {/* رأس */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px', flexWrap: 'wrap', gap: '6px' }}>
                          <div>
                            <div style={{ fontWeight: 'bold', fontSize: '15px', color: '#4a3010', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                              🌾 {feed.name}
                              <span style={{ background: '#f5f5f5', color: confidenceColor, fontSize: '11px', padding: '2px 8px', borderRadius: '6px', fontWeight: 'bold', border: `1px solid ${confidenceColor}` }}>{confidenceLabel}</span>
                              {feed.needsAlert && <span style={{ background: '#e67e22', color: 'white', fontSize: '11px', padding: '2px 7px', borderRadius: '6px', fontWeight: 'bold' }}>⚠️ اشتري قريباً!</span>}
                            </div>
                            <div style={{ fontSize: '12px', color: '#888', marginTop: '3px' }}>
                              {feed.purchases.length} عملية شراء · {feed.intervals.length} فترة محسوبة
                            </div>
                          </div>
                          {hasEnoughData && feed.avgDailyBags && (
                            <div style={{ textAlign: 'center', background: '#faf3e8', borderRadius: '8px', padding: '6px 12px', border: '1px solid #e8d5b0' }}>
                              <div style={{ fontWeight: 'bold', color: '#6e4b1f', fontSize: '16px' }}>{feed.avgDailyBags.toFixed(2)}</div>
                              <div style={{ fontSize: '10px', color: '#888' }}>{feed.unit}/يوم</div>
                            </div>
                          )}
                        </div>

                        {!hasEnoughData ? (
                          <div style={{ background: '#f9f9f9', borderRadius: '8px', padding: '10px', textAlign: 'center', color: '#aaa', fontSize: '12px' }}>
                            يحتاج عمليتي شراء على الأقل لحساب الاستهلاك الفعلي
                          </div>
                        ) : (
                          <>
                            {/* إحصائيات */}
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(100px,1fr))', gap: '7px', marginBottom: '10px' }}>
                              <div style={{ background: '#faf3e8', borderRadius: '7px', padding: '8px', textAlign: 'center' }}>
                                <div style={{ fontWeight: 'bold', color: '#6e4b1f', fontSize: '14px' }}>{feed.avgDailyBags?.toFixed(2)}</div>
                                <div style={{ fontSize: '10px', color: '#888' }}>{feed.unit}/يوم</div>
                              </div>
                              <div style={{ background: '#faf3e8', borderRadius: '7px', padding: '8px', textAlign: 'center' }}>
                                <div style={{ fontWeight: 'bold', color: '#e67e22', fontSize: '14px' }}>{feed.avgDailyKg?.toFixed(2)}</div>
                                <div style={{ fontSize: '10px', color: '#888' }}>كجم/يوم</div>
                              </div>
                              {feed.daysUntilNext !== null && (
                                <div style={{ background: feed.needsAlert ? '#fff3e0' : '#d5f5e3', borderRadius: '7px', padding: '8px', textAlign: 'center', border: `1px solid ${barColor}` }}>
                                  <div style={{ fontWeight: 'bold', color: barColor, fontSize: '14px' }}>{feed.daysUntilNext} يوم</div>
                                  <div style={{ fontSize: '10px', color: '#888' }}>المتبقي</div>
                                </div>
                              )}
                              {feed.weeklyCost && <div style={{ background: '#faf3e8', borderRadius: '7px', padding: '8px', textAlign: 'center' }}><div style={{ fontWeight: 'bold', color: '#c0392b', fontSize: '14px' }}>{feed.weeklyCost.toFixed(0)} ر</div><div style={{ fontSize: '10px', color: '#888' }}>أسبوعي</div></div>}
                              {feed.monthlyCost && <div style={{ background: '#faf3e8', borderRadius: '7px', padding: '8px', textAlign: 'center' }}><div style={{ fontWeight: 'bold', color: '#c0392b', fontSize: '14px' }}>{feed.monthlyCost.toFixed(0)} ر</div><div style={{ fontSize: '10px', color: '#888' }}>شهري</div></div>}
                              {feed.yearlyCost && <div style={{ background: '#faf3e8', borderRadius: '7px', padding: '8px', textAlign: 'center' }}><div style={{ fontWeight: 'bold', color: '#c0392b', fontSize: '14px' }}>{feed.yearlyCost.toFixed(0)} ر</div><div style={{ fontSize: '10px', color: '#888' }}>سنوي</div></div>}
                            </div>
                  ...
                  {/* ملخص تكاليف ذكي */}
                  {smartConsumption.some(f => f.monthlyCost) && (
                    <div style={{ background: 'linear-gradient(135deg, #6e4b1f, #4a3010)', borderRadius: '10px', padding: '14px', color: 'white', marginTop: '5px' }}>
                      <div style={{ fontSize: '13px', opacity: 0.9, marginBottom: '10px', fontWeight: 'bold' }}>💰 إجمالي تكاليف الأعلاف (حساب ذكي)</div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px' }}>
                        {[
                          { label: 'أسبوعي', val: smartConsumption.reduce((s, f) => s + (f.weeklyCost || 0), 0) },
                          { label: 'شهري', val: smartConsumption.reduce((s, f) => s + (f.monthlyCost || 0), 0) },
                          { label: 'سنوي', val: smartConsumption.reduce((s, f) => s + (f.yearlyCost || 0), 0) },
                        ].map(s => (
                          <div key={s.label} style={{ background: 'rgba(255,255,255,0.15)', borderRadius: '8px', padding: '8px', textAlign: 'center' }}>
                            <div style={{ fontWeight: 'bold', fontSize: '15px' }}>{s.val.toFixed(0)} ر</div>
